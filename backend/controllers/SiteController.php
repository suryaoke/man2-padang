...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        WHERE MONTH(kirim_at ) IN ('01','02','03','04','05','06') AND YEAR(kirim_at ) IN ('$thn') 
        GROUP BY MONTH(kirim_at) , YEAR(kirim_at) ORDER BY YEAR(kirim_at) DESC , MONTH(kirim_at) ASC")->queryAll();


        $semester2 = $DB->createCommand("SELECT *,COUNT(kirim_at) AS jumlah FROM `informasisurat`
        WHERE MONTH(kirim_at ) IN ('07','08','09','10','11','12') AND YEAR(kirim_at ) IN ('$thn') 
        GROUP BY MONTH(kirim_at) , YEAR(kirim_at) ORDER BY YEAR(kirim_at) DESC , MONTH(kirim_at) ASC")->queryAll();
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->bilangan_hasil == $datahasil) {
                ...
                $model->addError('bilangan_hasil', 'Hasil pertambahan salah');
            }
       // return $this->goBack();
        }
